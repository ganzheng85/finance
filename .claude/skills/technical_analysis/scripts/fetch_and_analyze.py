...
def fetch_stock_data(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    end_date = datetime.now() + timedelta(days=1)  # Add 1 day since end is exclusive
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"WARNING: Failed to fetch data from Yahoo Finance for {ticker}")
        print(f"{'='*70}")
        print(f"Error details: {str(e)}")
        print(f"\nPossible causes:")
        print(f"  - Network connection issue")
        print(f"  - Invalid ticker symbol '{ticker}'")
        print(f"  - Stock may be delisted or suspended from trading")
        print(f"  - Yahoo Finance API temporarily unavailable")
        print(f"  - Rate limiting (too many requests in short time)")
        print(f"\nPlease verify:")
        print(f"  1. Ticker symbol is correct (use Yahoo Finance format)")
        print(f"  2. Internet connection is working")
        print(f"  3. Try again in a few minutes if rate limited")
        print(f"{'='*70}\n")
        raise
