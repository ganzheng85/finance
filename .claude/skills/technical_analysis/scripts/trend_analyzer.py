...
class TrendAnalyzer:
    ...
    def analyze_trend_direction(self) -> Dict:
        ...
        dict
            {
                ...
                'price_trend': 'INCREASING', 'DECLINING', or 'SIDEWAYS',
        ...
        # Analyze price trend over the analysis period
        price_trend = 'SIDEWAYS'
        if len(self.df) >= 5:
            first_price = self.df['adjusted_close'].iloc[0]
            last_price = self.df['adjusted_close'].iloc[-1]
        ...
        if 'volume' in self.df.columns and len(self.df) >= 5:
            ...
            if recent_vol.iloc[-1] > recent_vol.mean() * 1.2:
                volume_trend = 'INCREASING'
            elif recent_vol.iloc[-1] < recent_vol.mean() * 0.8:
    ...
    def analyze_volatility(self) -> Dict:
        ...
        if 'atr_14' in self.df.columns and pd.notna(self.latest['atr_14']):
            atr = self.latest['atr_14']
            result['atr'] = atr
            # ATR as percentage of current price (for position sizing/stop-loss)
            if 'adjusted_close' in self.latest and pd.notna(self.latest['adjusted_close']):
                result['atr_pct'] = (atr / self.latest['adjusted_close']) * 100
    ...
    def identify_key_levels(self) -> Dict:
        ...
        # Detect swing highs and lows (local extrema)
        if len(self.df) >= 20:
            window = 10
            price_series = self.df['adjusted_close'].values
            high_series = self.df['high'].values if 'high' in self.df.columns else price_series
            low_series = self.df['low'].values if 'low' in self.df.columns else price_series
            ...
            for i in range(window, len(price_series) - window):
                ...
                if high_series[i] == max(high_series[i-window:i+window+1]):
                    swing_highs.append(high_series[i])
                # Swing low: local minimum
                if low_series[i] == min(low_series[i-window:i+window+1]):
                    swing_lows.append(low_series[i])

            # Cluster nearby levels (within 2% of each other)
            def cluster_levels(level_list, tolerance=0.02):
                if not level_list:
                    return []
                ...
                for level in sorted_levels[1:]:
                    if (level - current_cluster[-1]) / current_cluster[-1] <= tolerance:
                        ...
                        clusters.append(sum(current_cluster) / len(current_cluster))  # mean
                        current_cluster = [level]
                clusters.append(sum(current_cluster) / len(current_cluster))
                return clusters

            # Add clustered swing levels
            resistance_swings = cluster_levels([h for h in swing_highs if h > price])
            support_swings = cluster_levels([l for l in swing_lows if l < price])

            # Add to levels (limit to closest 2 swing levels)
            for swing_r in sorted(resistance_swings)[:2]:
                if swing_r < price * 1.20:  # Within 20% above
                    levels['resistance'].append({
                        'level': swing_r,
                        'type': 'Swing High',
                        'distance_pct': (swing_r - price) / price * 100
                    })

            for swing_s in sorted(support_swings, reverse=True)[:2]:
                if swing_s > price * 0.80:  # Within 20% below
                    levels['support'].append({
                        'level': swing_s,
                        'type': 'Swing Low',
                        'distance_pct': (price - swing_s) / price * 100
                    })
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Price Trend: {trend['price_trend']}")
        report.append(f"Volume Trend: {trend['volume_trend']}")
        report.append("")
