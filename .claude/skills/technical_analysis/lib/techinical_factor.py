...
class TechnicalFactors:
    ...
    def atr(self, window: int = 14) -> pd.DataFrame:
        """Average True Range (ATR) - requires 'high' and 'low' columns."""
        if 'high' not in self.df.columns or 'low' not in self.df.columns:
            raise ValueError("ATR requires 'high' and 'low' columns in the DataFrame")
        self.df = calculate_atr(self.df, window=window, price_col=self.price_col)
        return self.df
    ...
    def run_all(self,
        ...
        # IMPORTANT: Set keep_components=True to save bb_upper, bb_lower, bb_middle for charting
        bb_params = params.get('bollinger_width', {})
        bb_params.setdefault('keep_components', True)  # Force keep_components=True
        self.bollinger_width(**bb_params)
...
def calculate_atr(
    ...
    """
    Calculate Average True Range (ATR) per symbol on a long-format DataFrame.
    Requires columns: ['symbol', 'date', 'high', 'low', price_col].

    ATR measures volatility and is useful for:
    - Setting stop-loss distances
    - Position sizing
    - Identifying volatility changes

    Outputs column:
      + 'atr_{window}' : Average True Range (Wilder's smoothing)
    """
    df = df.sort_values(["symbol", "date"]).copy()
